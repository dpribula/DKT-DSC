...
def run_epoch(session, m, students, cluster, run_type, eval_op, verbose=False):
          ...
          if (len(problem_ids)>limit):
             for i in range(m.batch_size):
                 for j in range(m.num_steps):
                     ...
                     if target_indx < len(problem_ids)-1:
...
def read_data_from_csv_file(path):
    ...
    max_steps= (max_attempts/FLAGS.batch_size)+1
    ...
    with tf.Graph().as_default():
         ...
         with tf.Session(config=session_conf) as session:              
              ...
              for i in range(config.max_max_epoch):
                  rmse, auc, r2, _ = run_epoch(session, m, train_students, cluster, 'train', train_op, verbose=False)
                  print("Epoch: %d Train Metrics:\n rmse: %.3f \t auc: %.3f \t r2: %.3f \n" % (i + 1, rmse, auc, r2))
                  if((i+1) % FLAGS.evaluation_interval == 0):
                     rmse, auc, r2, all_logits = run_epoch(session, mtest, test_students, cluster, 'test', tf.no_op(), verbose=True)
